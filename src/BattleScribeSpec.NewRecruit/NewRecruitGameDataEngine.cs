...
public sealed class NewRecruitGameDataEngine : IGameDataEngine
{
    ...
    {
        MoveEntryAsync(entryId, newParentId, index).GetAwaiter().GetResult();
    }

    private async Task MoveEntryAsync(string entryId, string newParentId, int? index)
    {
        if (_page is null)
        { throw new InvalidOperationException("Page not initialized"); }
        ...
        var result = await _page.EvaluateAsync<string?>("""
            ([entryId, newParentId, index]) => {
                try {
                    ...
                    // Find entry and its current parent
                    const findWithParent = (parent, id) => {
                        ...
                        for (const key of Object.keys(parent)) {
                            ...
                            if (Array.isArray(val)) {
                                for (let i = 0; i < val.length; i++) {
                                    if (val[i]?.id === id) {
                                        return { entry: val[i], parent, key, index: i };
                                    }
                                    const found = findWithParent(val[i], id);
                                    if (found) return found;
                                }
                            }
                        }
                    ...
                    const found = findWithParent(catalogue, entryId);
                    if (!found) return 'Entry not found: ' + entryId;
                    ...
                    const newParent = findById(catalogue, newParentId);
                    if (!newParent) return 'New parent not found: ' + newParentId;

                    // Remove from old location
                    found.parent[found.key].splice(found.index, 1);

                    // Add to new parent (same container key)
                    if (!newParent[found.key]) newParent[found.key] = [];
                    const insertIdx = index !== null && index >= 0
                        ? Math.min(index, newParent[found.key].length)
                        : newParent[found.key].length;
                    newParent[found.key].splice(insertIdx, 0, found.entry);
