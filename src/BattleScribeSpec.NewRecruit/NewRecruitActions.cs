...
public static class NewRecruitActions
{
    ...
    /// </summary>
    public static async Task SelectEntryByNameAsync(IPage page, int[] forcePath, string entryName)
    {
        var error = await page.EvaluateAsync<string?>("""
            ({forcePath, entryName}) => {
                try {
    ...
    /// </summary>
    public static async Task SelectChildEntryByNameAsync(IPage page, int[] forcePath, int[] selectionPath, string childEntryName)
    {
        var error = await page.EvaluateAsync<string?>("""
            ({forcePath, selectionPath, childEntryName}) => {
                try {
                    const army = window.__bsspec?.army;
                    if (!army) return 'No current roster';
            ...
            """, new { forcePath, selectionPath, childEntryName });
        if (error != null) throw new InvalidOperationException(error);
    }
    ...
    /// </summary>
    public static async Task SetSelectionCountAsync(IPage page, int[] forcePath, int entryIndex, int count)
    {
        var error = await page.EvaluateAsync<string?>("""
            ({forcePath, entryIndex, count}) => {
                try {
                    const army = window.__bsspec?.army;
                    if (!army) return 'No current roster';
            ...
            """, new { forcePath, entryIndex, count });
        if (error != null) throw new InvalidOperationException(error);
    }
