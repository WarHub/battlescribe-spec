...
public sealed class BsUiDiagnostics
{
    ...
    /// <summary>
    /// How long each diagnostic call waits.
    /// </summary>
    /// <remarks>
    /// Short, because this runs after something already went wrong and the agent may be half-stuck:
    /// a partial dump now beats a complete one after another 30s per section. Passed per call rather
    /// than assigned to <see cref="AgentClient.CallTimeout"/> — the client belongs to the engine, and
    /// a capture that faults before restoring it would leave every later call on a 5s clock.
    /// </remarks>
